...
internal class SortHelper {
    ...
    private fun mergeSort(src: IntArray, dest: IntArray, paramLow: Int, paramHigh: Int, off: Int) {
        ...
        if (src[mid - 1] <= src[mid]) {
            src.copyInto(dest, destLow, low, low + length)
            return
        }
    ...
    companion object {
        ...
        private fun rangeCheck(arrayLen: Int, from: Int, to: Int) {
            ...
            if (from < 0) {
                throw IndexOutOfBoundsException(from.toString())
            }
            if (to > arrayLen) {
                throw IndexOutOfBoundsException(to.toString())
            }
        }
    }
