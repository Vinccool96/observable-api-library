...
import io.github.vinccool96.observable.internal.utils.SortedSet
...
internal abstract class ChangeBuilder<T> {
    ...
    internal class SubChange<T>(var from: Int, var to: Int, var removed: MutableList<T>?, var perm: IntArray,
        ...
        override fun toString(): String {
            return "SubChange(from=$from, to=$to, removed=$removed, perm=${perm.contentToString()}, updated=$updated)"
        }
    ...
    fun nextPermutation(from: Int, to: Int, perm: IntArray) {
        ...
        if (this.addRemoveChanges.isNotEmpty()) {
            ...
            val removed: MutableSet<Int> = SortedSet()
            var last = 0
            var offset = 0
            run {
                var i = 0
        ...
        if (this.addRemoveChanges.isNotEmpty()) {
            val newAdded: MutableSet<Int> = SortedSet()
        ...
        if (this.updateChanges.isNotEmpty()) {
            val newUpdated: MutableSet<Int> = SortedSet()
            var i = 0
    ...
    companion object {
        ...
        internal fun <T> finalizeSubChange(c: SubChange<T>): SubChange<T> {
            if (c.removed == null) {
                ...
                c.removed = ObservableCollections.unmodifiableObservableList(
                        ObservableCollections.observableList(c.removed!!))
            }
            return c
        }
    }
