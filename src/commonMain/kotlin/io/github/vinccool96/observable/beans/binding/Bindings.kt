...
import io.github.vinccool96.observable.internal.binding.*
 ...
 * [io.github.vinccool96.observable.beans.binding]). (Every Expression contains a static method that generates an
...
object Bindings {
     ...
     *
     * @param func The function that calculates the value of this binding. If it returns `null`, the binding will
     * contain `false`
        ...
        return object : BooleanBinding() {
            ...
            override fun computeValue(): Boolean {
                return try {
                    ...
                    Logging.logger.warn("Exception while evaluating binding", e)
                    false
                }
            }
    ...
    // TODO createMapBinding

    // TODO createSetBinding
    ...
    // TODO bindContentBidirectional for Map

    // TODO bindContentBidirectional for Set

    /**
     * Remove a bidirectional content binding.
     *
     * @param obj1 the first `Any`
     * @param obj2 the second `Any`
     */
    ...
    // TODO bindContent for Map

    // TODO bindContent for Set
    ...
    private class ShortCircuitAndInvalidator(binding: BooleanAndBinding) : InvalidationListener {
        ...
        private val ref: WeakReference<BooleanAndBinding> = WeakReferenceFactory.createWeakRef(binding)
    ...
    private class ShortCircuitOrInvalidator(binding: BooleanOrBinding) : InvalidationListener {
        ...
        private val ref: WeakReference<BooleanOrBinding> = WeakReferenceFactory.createWeakRef(binding)
    ...
    fun format(stringMaker: () -> String, vararg args: Any): StringExpression {
        return StringFormatter.format(stringMaker, *args)
    }
    ...
    private fun equal(op1: ObservableStringValue, op2: ObservableStringValue,
        ...
        return object : BooleanBinding() {
            ...
            @get:ReturnsUnmodifiableCollection
            override val dependencies: ObservableList<Observable>
                get() = if (dependencies.size == 1) ObservableCollections.singletonObservableList(
                        dependencies[0]) else ImmutableObservableList(*dependencies)
        ...
        return object : ObjectBinding<E?>() {
            ...
            override fun computeValue(): E? {
                try {
                    ...
                    Logging.logger.info("Exception while evaluating binding", ex)
                }
                return null
            }
        ...
        return object : BooleanBinding() {
            ...
            override fun computeValue(): Boolean {
                try {
                    val value = op[index]
                    if (value == null) {
                        Logging.logger.info("List element is null, returning default value instead.",
