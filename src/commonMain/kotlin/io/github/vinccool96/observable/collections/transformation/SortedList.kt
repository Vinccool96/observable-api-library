 ...
 *
 * @constructor Creates a new SortedList wrapped around the source list. The source list will be sorted using the
 * comparator provided. If `null` is provided, the list stays unordered and is equal to the source list.
 *
 * @param source a list to wrap
 * @param comparator a comparator to use or `null` for unordered List
 */
@Suppress("UNCHECKED_CAST")
open class SortedList<E>(@NamedArg("source") source: ObservableList<E>,
        @NamedArg("comparator") comparator: Comparator<in E>?) : TransformationList<E, E>(source) {

    private var elementComparator: Comparator<Element<E>>? = null

    private var sorted: Array<Element<E>?> = arrayOfNulls<Element<E>?>(source.size * 3 / 2 + 1)

    private var perm: IntArray = IntArray(sorted.size)
    ...
    private class ElementComparator<E>(val comparator: Comparator<E>) : Comparator<Element<E>> {
        ...
        override fun compare(a: Element<E>, b: Element<E>): Int {
            return this.comparator.compare(a.e, b.e)
        }
    ...
    private fun findPosition(e: E): Int {
        ...
        return this.sorted.toMutableList().binarySearch(this.tempElement,
                this.elementComparator!! as Comparator<in Element<E>?>, 0, this.sizeState)
    }
