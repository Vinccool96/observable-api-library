...
abstract class ModifiableObservableListBase<E> : ObservableListBase<E>() {
    ...
    @Transient
    protected var modificationCount = 0

    override fun clear() {
        removeTheRange(0, this.size)
    }
    ...
    override fun removeTheRange(fromIndex: Int, toIndex: Int) {
        beginChange()
        try {
            super.removeTheRange(fromIndex, toIndex)
        } finally {
            endChange()
        }
    }
    ...
    override fun removeAll(elements: Collection<E>): Boolean {
        beginChange()
        try {
            return super.removeAll(elements.toList())
        } finally {
            endChange()
        }
    }
