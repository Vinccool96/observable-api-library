...
class ListBindingTest {
    ...
    private class ListChangeListenerMock : ListChangeListener<Any> {
        ...
        private var capturedChange: Change<out Any>? = null
        ...
        override fun onChanged(change: Change<out Any>) {
            ...
            } while (this.capturedChange == null)
            this.counter++
        }

        fun reset() {
            this.capturedChange = null
            this.counter = 0
        }

        fun checkNotCalled() {
            assertEquals(null, this.capturedChange)
            assertEquals(0, this.counter)
            reset()
        }

        fun check(oldList: ObservableList<Any>, newList: ObservableList<Any>, counter: Int) {
            val change = this.capturedChange
            if (change != null) {
                assertTrue(change.next())
                assertTrue(change.wasReplaced)
                assertEquals(oldList, change.removed)
                assertEquals(change.list, newList)
                assertFalse(change.next())
                assertEquals(counter, this.counter)
                reset()
            } else {
                fail("change shouldn't be null")
            }
        }

        fun check(pos: Int, newObject: Any, counter: Int) {
            assertTrue(this.capturedChange!!.next())
            assertTrue(this.capturedChange!!.wasAdded)
            assertEquals(pos, this.capturedChange!!.from)
            assertEquals(listOf(newObject), this.capturedChange!!.addedSubList)
            assertFalse(this.capturedChange!!.next())
            assertEquals(counter, this.counter)
            reset()
        }
